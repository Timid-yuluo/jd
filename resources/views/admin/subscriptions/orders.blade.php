        ...
        <div class="card-actions">
            <form method="GET" class="d-flex gap-2 flex-wrap">
                ...
                @if(request()->hasAny(['search','status']))
                    <a href="{{ route('admin.subscriptions.orders') }}" class="btn btn-sm btn-secondary">重置</a>
                @endif
            </form>
        </div>
    </div>
            ...
            <tbody>
                @forelse($orders as $order)
                    <tr>
                        ...
                        <td>{{ $order->billing_cycle === 'yearly' ? '年付' : '月付' }}</td>
                        <td>
                            @php
                ...
                @empty
                    <tr><td colspan="7" class="text-center py-4 text-secondary">暂无订单</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
