...
@section('title', '编辑赛道 - ' . $careerTrack->name)
@section('page-pretitle', '校招赛道管理')
@section('page-title', '编辑赛道')

@section('content')
<form action="{{ route('admin.career-tracks.update', $careerTrack) }}" method="POST">
    @csrf @method('PUT')
    <div class="row g-3">
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control" value="{{ old('name', $careerTrack->name) }}" maxlength="50" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label required">赛道标识</label>
                        <input type="text" name="slug" class="form-control" value="{{ old('slug', $careerTrack->slug) }}" maxlength="50" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label required">分类</label>
                        <select name="category" class="form-select" required>
                            @foreach($categories as $key => $label)
                                <option value="{{ $key }}" {{ old('category', $careerTrack->category) === $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">图标</label>
                        <input type="text" name="icon" class="form-control" value="{{ old('icon', $careerTrack->icon) }}" maxlength="50">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">主题色</label>
                        <input type="text" name="color" class="form-control" value="{{ old('color', $careerTrack->color) }}" maxlength="20">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">描述</label>
                        <textarea name="description" class="form-control" rows="3" maxlength="500">{{ old('description', $careerTrack->description) }}</textarea>
                    </div>
                    <div class="mb-3">
                        ...
                        <select name="prompt_strategy_key" class="form-select">
                            ...
                            @foreach($strategyKeys as $key)
                                <option value="{{ $key }}" {{ old('prompt_strategy_key', $careerTrack->prompt_strategy_key) === $key ? 'selected' : '' }}>{{ $key }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="row g-3">
                        <div class="col-6">
                            ...
                            <input type="number" name="sort_order" class="form-control" value="{{ old('sort_order', $careerTrack->sort_order) }}" min="0">
                        </div>
                        <div class="col-6">
                            <label class="form-label">状态</label>
                            <div class="form-selectgroup">
                                <label class="form-selectgroup-item">
                                    <input type="radio" name="is_active" value="1" class="form-selectgroup-input" {{ old('is_active', $careerTrack->is_active) ? 'checked' : '' }}>
                                    <span class="form-selectgroup-label">启用</span>
                                </label>
                                <label class="form-selectgroup-item">
                                    <input type="radio" name="is_active" value="0" class="form-selectgroup-input" {{ !old('is_active', $careerTrack->is_active) ? 'checked' : '' }}>
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <div id="focus-container">
                        @foreach(old('optimization_focus', $careerTrack->optimization_focus ?? []) as $idx => $focus)
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <div id="templates-container">
                        @foreach(old('recommended_templates', $careerTrack->recommended_templates ?? []) as $idx => $tpl)
                            <div class="input-group mb-2">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <div id="keywords-container">
                        @foreach(old('keywords', $careerTrack->keywords ?? []) as $idx => $kw)
                            <div class="input-group mb-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div id="avoid-container">
                        @foreach(old('avoid_words', $careerTrack->avoid_words ?? []) as $idx => $word)
                            <div class="input-group mb-2">
    ...
    <div class="mt-3">
        <button type="submit" class="btn btn-primary"><i class="ti ti-check me-1"></i>保存修改</button>
