<div class="visual-resume" :class="`template-${template}`" :style="`font-size: var(--body-fs, var(--rs-font-size, 13.5px)); line-height: var(--body-lh, var(--rs-line-height, 1.7)); color: var(--body-c, #374151); padding: 28px 26px; --heading-fs: var(--rs-heading-font-size, 1em); --heading-c: var(--rs-heading-color, var(--accent-color, #b45309)); --accent-c: var(--rs-accent-color, var(--accent-color, #b45309)); --section-mb: var(--rs-section-spacing, 18px); --body-c: var(--rs-body-color, #374151); --body-fs: var(--rs-font-size, 13.5px); --body-lh: var(--rs-line-height, 1.7);`">
    <template x-for="(mod, modIdx) in modules.filter(m => m.type === 'personal')" :key="mod._key">
        <div style="display: flex; align-items: center; gap: 18px; margin-bottom: 20px; padding-bottom: 18px; border-bottom: 2px solid var(--accent-c);" :data-mod-index="modules.findIndex(m => m._key === mod._key)" :style="`display: flex; align-items: center; gap: 18px; margin-bottom: 20px; padding-bottom: 18px; border-bottom: 2px solid var(--accent-c); ${moduleStyleOverride(mod)}`">
            <div x-show="mod.data.avatar" style="flex-shrink: 0;">
                <img :src="mod.data.avatar" alt="头像" style="width: 68px; height: 68px; border-radius: 8px; object-fit: cover; border: 2px solid var(--accent-c);">
            </div>
            <div style="flex: 1; min-width: 180px;">
                ...
                <div style="display: flex; gap: 14px; flex-wrap: wrap; font-size: 0.88em; color: var(--body-c); opacity: 0.7;">
    ...
    <template x-for="(mod, modIdx) in modules.filter(m => m.type === 'objective')" :key="mod._key">
        <div style="background: var(--accent-bg, #fffbeb); border-left: 4px solid var(--accent-c); padding: 12px 18px; margin-bottom: var(--section-mb); border-radius: 0 8px 8px 0;" :data-mod-index="modules.findIndex(m => m._key === mod._key)" :style="`background: var(--accent-bg, #fffbeb); border-left: 4px solid var(--accent-c); padding: 12px 18px; margin-bottom: var(--section-mb); border-radius: 0 8px 8px 0; ${moduleStyleOverride(mod)}`">
    ...
    <div :style="dualColumnGridStyle('16px')">
        <template x-for="(mod, modIdx) in modules.filter(m => !['personal','objective'].includes(m.type))" :key="mod._key">
            <div :style="moduleGridColumnStyle(mod, { template: 'teacher', defaultFullTypes: ['summary'] }) + ' ' + moduleStyleOverride(mod)" :data-mod-index="modules.findIndex(m => m._key === mod._key)">
                <template x-if="mod.type === 'education'">
                    <div style="margin-bottom: var(--section-mb);">
                        <h3 style="margin: 0 0 8px 0; font-size: var(--heading-fs); font-weight: var(--rs-title-weight, 700); color: var(--accent-c); display: flex; align-items: center; gap: 6px;">
                            <span style="display: inline-block; width: 20px; height: 20px; background: var(--accent-c); border-radius: 4px; color: #fff; text-align: center; line-height: 20px; font-size: 0.7em;">📚</span>
                ...
                <template x-if="['experience','project','certificate'].includes(mod.type)">
                    <div style="margin-bottom: var(--section-mb);">
                        <h3 style="margin: 0 0 8px 0; font-size: var(--heading-fs); font-weight: var(--rs-title-weight, 700); color: var(--accent-c); display: flex; align-items: center; gap: 6px;">
                            <span style="display: inline-block; width: 20px; height: 20px; background: var(--accent-c); border-radius: 4px; color: #fff; text-align: center; line-height: 20px; font-size: 0.7em;">✏️</span>
                    ...
                    <div style="margin-bottom: var(--section-mb);">
                        ...
                        <div class="inline-edit-list" style="display: flex; flex-wrap: wrap; gap: 6px;">
                            <template x-for="(item, i) in mod.data.items" :key="i">
                                <span class="inline-item-wrap" x-show="item !== undefined" style="display: inline-flex; align-items: center; padding: 4px 12px; background: var(--accent-bg, #fffbeb); color: var(--accent-c); border-radius: 20px; font-size: 0.85em; font-weight: 500;">
                ...
                <template x-if="mod.type === 'summary'">
                    <div style="margin-bottom: var(--section-mb); background: var(--accent-bg, #fffbeb); border-radius: 8px; padding: 14px 18px; border-left: 4px solid var(--accent-c);">
    ...
    .theme-blue { --accent-color: #b45309; --accent-bg: #fffbeb; }
