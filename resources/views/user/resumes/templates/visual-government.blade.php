<div class="visual-resume" :class="`template-${template}`" :style="`font-size: var(--body-fs, var(--rs-font-size, 13.5px)); line-height: var(--body-lh, var(--rs-line-height, 1.7)); color: var(--body-c, #1f2937); padding: 28px 26px; --heading-fs: var(--rs-heading-font-size, 1em); --heading-c: var(--rs-heading-color, var(--accent-color, #b91c1c)); --accent-c: var(--rs-accent-color, var(--accent-color, #b91c1c)); --section-mb: var(--rs-section-spacing, 18px); --body-c: var(--rs-body-color, #1f2937); --body-fs: var(--rs-font-size, 13.5px); --body-lh: var(--rs-line-height, 1.7);`">
    <template x-for="(mod, modIdx) in modules.filter(m => m.type === 'personal')" :key="mod._key">
        <div style="margin-bottom: 22px; padding-bottom: 16px; border-bottom: 3px double var(--accent-c);" :data-mod-index="modules.findIndex(m => m._key === mod._key)" :style="`margin-bottom: 22px; padding-bottom: 16px; border-bottom: 3px double var(--accent-c); ${moduleStyleOverride(mod)}`">
            <div style="display: flex; align-items: center; gap: 18px;">
                <div x-show="mod.data.avatar" style="flex-shrink: 0;">
                    <img :src="mod.data.avatar" alt="头像" style="width: 64px; height: 64px; border-radius: 50%; object-fit: cover; border: 2px solid var(--accent-c);">
                </div>
                <div style="flex: 1;">
                    <h1 style="margin: 0 0 6px 0; font-size: 1.72em; font-weight: 700; color: var(--accent-c);">
    ...
    <template x-for="(mod, modIdx) in modules.filter(m => m.type === 'objective')" :key="mod._key">
        <div style="margin-bottom: var(--section-mb); padding: 12px 18px; background: var(--accent-bg, #fef2f2); border: 1px solid var(--accent-c); border-radius: 8px;" :data-mod-index="modules.findIndex(m => m._key === mod._key)" :style="`margin-bottom: var(--section-mb); padding: 12px 18px; background: var(--accent-bg, #fef2f2); border: 1px solid var(--accent-c); border-radius: 8px; ${moduleStyleOverride(mod)}`">
            <span style="font-size: 0.82em; font-weight: 600; color: var(--accent-c);">🏛️ 求职意向</span>
    ...
    <div :style="dualColumnGridStyle('16px')">
        <template x-for="(mod, modIdx) in modules.filter(m => !['personal','objective'].includes(m.type))" :key="mod._key">
            <div :style="moduleGridColumnStyle(mod, { template: 'government', defaultFullTypes: ['summary'] }) + ' ' + moduleStyleOverride(mod)" :data-mod-index="modules.findIndex(m => m._key === mod._key)">
                <template x-if="mod.type === 'education'">
                    <div style="margin-bottom: var(--section-mb);">
                        <h3 style="margin: 0 0 8px 0; font-size: var(--heading-fs); font-weight: var(--rs-title-weight, 700); color: var(--accent-c); border-bottom: 1px solid var(--accent-c); padding-bottom: 4px;">
                ...
                <template x-if="['experience','project','certificate'].includes(mod.type)">
                    <div style="margin-bottom: var(--section-mb);">
                        ...
                        <div style="font-size: 0.89em; color: var(--body-c); opacity: 0.65; margin-bottom: 6px; display: flex; gap: 10px; flex-wrap: wrap;">
                            <span style="font-weight: 600;"><span class="inline-edit" contenteditable="true" data-placeholder="单位" @input="inlineUpdate(modules.findIndex(m => m._key === mod._key), 'subtitle', $event.target.innerText)" x-text="mod.data.subtitle || ''"></span></span>
                            <span x-show="mod.data.date"><span class="inline-edit" contenteditable="true" data-placeholder="时间" @input="inlineUpdate(modules.findIndex(m => m._key === mod._key), 'date', $event.target.innerText)" x-text="mod.data.date || ''"></span></span>
                        </div>
                    ...
                    <div style="margin-bottom: var(--section-mb);">
                        ...
                        <div class="inline-edit-list" style="display: flex; flex-wrap: wrap; gap: 6px;">
                            <template x-for="(item, i) in mod.data.items" :key="i">
                                <span class="inline-item-wrap" x-show="item !== undefined" style="display: inline-flex; align-items: center; padding: 4px 12px; background: var(--accent-bg, #fef2f2); color: var(--accent-c); border-radius: 4px; font-size: 0.85em; font-weight: 500;">
                ...
                <template x-if="mod.type === 'summary'">
                    <div style="margin-bottom: var(--section-mb); background: var(--accent-bg, #fef2f2); border-radius: 8px; padding: 14px 18px; border: 1px solid var(--accent-c);">
    ...
    .theme-blue { --accent-color: #b91c1c; --accent-bg: #fef2f2; }
    .theme-coral { --accent-color: #9f1239; --accent-bg: #fff1f2; }
    .theme-green { --accent-color: #15803d; --accent-bg: #f0fdf4; }
    .theme-purple { --accent-color: #6d28d9; --accent-bg: #f5f3ff; }
    .theme-orange { --accent-color: #9a3412; --accent-bg: #fff7ed; }
    </style>
</div>
