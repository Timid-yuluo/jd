(() => {
        ...
        const sourceEl = document.getElementById('applyTemplateSource');
        const fromIdEl = document.getElementById('applyTemplateFromId');
        ...
        document.querySelectorAll('.js-open-apply-modal').forEach((button) => {
            button.addEventListener('click', () => {
                ...
                const fromTemplateId = button.getAttribute('data-from-template-id') || '';

                form.setAttribute('action', action);
                nameEl.textContent = templateName;
                sourceEl.value = source;
                fromIdEl.value = fromTemplateId;
                if (idempotencyEl) {
